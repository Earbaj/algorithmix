...
import 'package:algorithmix/domain/models/tree_bfs_data.dart';
import 'package:algorithmix/ui/core/theme/app_theme.dart';
import 'package:algorithmix/ui/core/utils/responsive.dart';
import '../widgets/tree_bfs_visualizer.dart';

class TreeBfsDetailScreen extends StatefulWidget {
  const TreeBfsDetailScreen({super.key});

  @override
  State<TreeBfsDetailScreen> createState() => _TreeBfsDetailScreenState();
}

class _TreeBfsDetailScreenState extends State<TreeBfsDetailScreen>
  ...
  Widget build(BuildContext context) {
    final intro = TreeBfsData.getConceptIntro(_isEnglish);

    return Scaffold(
      backgroundColor: AppTheme.primaryDark,
      appBar: AppBar(
        title: Text(_isEnglish ? 'Tree BFS Deep Dive' : 'ট্রি BFS (লেভেল অর্ডার) গাইড'),
        centerTitle: true,
        actions: [
          Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _buildCodeBox("Template 1: Standard Level Order Traversal (C++ LeetCode 102)", """
vector<vector<int>> levelOrder(TreeNode* root) {
    vector<vector<int>> res; if (!root) return res;
    queue<TreeNode*> q; q.push(root);
    while (!q.empty()) {
        int levelSize = q.size(); // Snapshot size!
        vector<int> level;
    ...
    return res;
            ...
            _buildCodeBox("Template 2: Binary Tree Zigzag Level Order (C++ LeetCode 103)", """
vector<vector<int>> zigzagLevelOrder(TreeNode* root) {
    ...
    return res;
            ...
            const SizedBox(height: 16),
            _buildCodeBox("Template 3: Binary Tree Right Side View (C++ LeetCode 199)", """
vector<int> rightSideView(TreeNode* root) {
  ...
  Widget _buildVisualizerTab() {
    ...
    return ResponsiveCenter(
      padding: EdgeInsets.all(hPadding),
      child: SingleChildScrollView(
        child: TreeBfsVisualizer(isEnglish: _isEnglish),
      ),
    );
  }
  ...
  Widget _buildProblemsTab() {
    final hPadding = Responsive.horizontalPadding(context);
    final easy = TreeBfsData.getEasyProblems();
    final medium = TreeBfsData.getMediumProblems();
    final hard = TreeBfsData.getHardProblems();
  ...
  Widget _buildProblemSection(String title, List<TreeBfsProblem> problems, Color diffColor) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
  ...
  Widget _buildProblemCard(TreeBfsProblem p, Color diffColor) {
    return Container(
  ...
  Widget _buildMistakesTab() {
    final hPadding = Responsive.horizontalPadding(context);
    final mistakes = TreeBfsData.getCommonMistakes(_isEnglish);
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _buildRoadmapStep(1, _isEnglish ? "Master FIFO Queue Mechanics" : "FIFO ক্যু মেকানিক্স আয়ত্তকরণ", _isEnglish ? "Understand queue push, pop, front operations." : "ক্যু এর push, pop ও front অপারেশন আয়ত্ত করুন।"),
            _buildRoadmapStep(2, _isEnglish ? "Master Level Size Snapshotting" : "লেভেল সাইজ স্ন্যাপশট নেওয়া", _isEnglish ? "Snapshot int levelSize = q.size() at start of each level." : "প্রতিটি লেভেলে `levelSize = q.size()` স্ন্যাপশট নিন।"),
            _buildRoadmapStep(3, _isEnglish ? "Master Level Aggregation & Averages" : "লেভেল অনুযায়ী এগ্রিগেশন ও গড়", _isEnglish ? "Compute max values or level averages across tree rows." : "লেভেল অনুযায়ী সর্বোচ্চ মান বা গড় হিসাব করুন।"),
            _buildRoadmapStep(4, _isEnglish ? "Master Zigzag & Right Side View Filtering" : "জিগজ্যাগ ও রাইট সাইড ভিউ ফিল্টারিং", _isEnglish ? "Manipulate level indices or filter rightmost node values." : "দিক পরিবর্তন ও রাইট সাইড নোড মেলান।"),
            _buildRoadmapStep(5, _isEnglish ? "Master Graph Level BFS & Tree Connections" : "গ্রাফ লেভেল BFS ও নোড সিবলিং জোড়া", _isEnglish ? "Connect nextRight node pointers or solve Word Ladder graph BFS." : "পরবর্তী লেভেল পয়েন্টার বা গ্রাফ BFS সমাধান করুন।"),
            const SizedBox(height: 24),
          ],
        ),
      ),
