...
class MinStackPracticeQuiz extends StatefulWidget {
  final bool isEnglish;

  const MinStackPracticeQuiz({
    super.key,
    required this.isEnglish,
  });

  @override
  State<MinStackPracticeQuiz> createState() => _MinStackPracticeQuizState();
}
...
class _MinStackPracticeQuizState extends State<MinStackPracticeQuiz> {
  int _activeSubTab = 0;
  ...
  final List<TestCaseData> _testCases = const [
    TestCaseData(inputStr: "push(-2), push(0), push(-3), getMin()", expectedOutputStr: "min = -3"),
    TestCaseData(inputStr: "pop(), getMin()", expectedOutputStr: "min = -2"),
    TestCaseData(inputStr: "push(5), top(), getMin()", expectedOutputStr: "top = 5, min = -2"),
  ];
  ...
  final List<QuizQuestion> _quizQuestions = const [
    QuizQuestion(
      questionEn: "1. How does Min Stack achieve O(1) time for `getMin()` without scanning the stack?",
      questionBn: "১. স্ট্যাক স্ক্যান না করে Min Stack কীভাবে O(1) টাইমে `getMin()` প্রদান করে?",
      optionsEn: [
        "A) By using an auxiliary stack (minStack) that maintains the current minimum at every height level!",
        "B) By sorting the main stack on every push",
        "C) By using binary search",
        "D) It scans the stack in O(N)"
      ],
      optionsBn: [
        "A) একটি সেকেন্ডারি minStack এর মাধ্যমে প্রতিটি উচ্চতায় (Height level) সর্বনিম্ন মান ধরে রেখে!",
        "B) প্রতি পুশে মেইন স্ট্যাক সোর্ট করে",
        "C) বাইনারি সার্চ করে",
        "D) O(N) এ পুরো স্ট্যাক খুঁজে"
      ],
      correctOptionIndex: 0,
      explanationEn: "Correct! The auxiliary minStack stores the minimum element corresponding to each stack level, so `minStack.top()` is always the overall minimum in O(1) time.",
      explanationBn: "সঠিক! অক্সিলিয়ারি minStack প্রতিটি স্তরের সর্বনিম্ন মান ধরে রাখায় `minStack.top()` সবসময় O(1) টাইমে ওভারঅল মিনিমাম মান প্রদান করে।",
    ),
    QuizQuestion(
      questionEn: "2. When popping an element from MinStack, why must we pop from BOTH `st` and `minSt`?",
      questionBn: "২. MinStack থেকে কোনো উপাদান পপ করার সময় কেন `st` ও `minSt` দুটি থেকেই পপ করতে হয়?",
      optionsEn: [
        "A) To keep both stacks perfectly synchronized in size and height levels!",
        "B) Only `st` needs to be popped",
        "C) To reset the stack",
        "D) Because minSt is a pointer"
      ],
      optionsBn: [
        "A) যাতে দুটি স্ট্যাকের সাইজ ও উচ্চতা সমান্তরাল ও সিনক্রোনাইজড থাকে!",
        "B) কেবল `st` পপ করলেই হয়",
        "C) স্ট্যাক রিসেট করতে",
        "D) কারণ minSt একটি পয়েন্টার"
      ],
      correctOptionIndex: 0,
      explanationEn: "Correct! Popping from both stacks ensures that when the minimum element is removed, the previous minimum below it is restored to the top of `minSt`.",
      explanationBn: "সঠিক! দুটি থেকেই পপ করার ফলে শীর্ষ সর্বনিম্ন মান সরে গেলে তার আগের সর্বনিম্ন মানটি অটোমেটিক `minSt` এর টপে ফেরত আসে।",
    ),
  ];
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Flexible(
                          child: Text(
                            widget.isEnglish ? "Min Stack Quiz" : "মিন স্ট্যাক কুইজ",
                            overflow: TextOverflow.ellipsis,
                            maxLines: 1,
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(
                    child: Text(
                      widget.isEnglish ? "Min Stack Test Runner" : "মিন স্ট্যাক টেস্ট রানার",
              ...
              Text(
                widget.isEnglish
                    ? "Verify O(1) getMin() performance across push, pop, duplicate minimums, and empty states."
                    : "পুশ, পপ ও পুনরাবৃত্ত নোডের ক্ষেত্রে O(1) মিনিমাম কুয়েরি সুবিধা ভেরিফাই করুন।",
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      widget.isEnglish ? "Min Stack Mastery Quiz" : "মিন স্ট্যাক মাস্টার কুইজ",
