...
class CyclicSortConceptTabView extends StatelessWidget {
  final Map<String, String> intro;
  final bool isEnglish;

  const CyclicSortConceptTabView({
    super.key,
    required this.intro,
...
int missingNumber(vector<int>& nums) {
    ...
    for (int j = 0; j < n; j++) {
        if (nums[j] != j) return j; // Missing number!
    }
    return n;
}"""),
            const SizedBox(height: 16),
            _buildCodeBox("Template 3: Find Duplicate Number (LeetCode 287)", """
