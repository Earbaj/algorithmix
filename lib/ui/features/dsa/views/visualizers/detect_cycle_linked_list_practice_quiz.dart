...
class DetectCycleLinkedListPracticeQuiz extends StatefulWidget {
  final bool isEnglish;

  const DetectCycleLinkedListPracticeQuiz({
    super.key,
    required this.isEnglish,
  });

  @override
  State<DetectCycleLinkedListPracticeQuiz> createState() =>
      _DetectCycleLinkedListPracticeQuizState();
}
...
class _DetectCycleLinkedListPracticeQuizState
    extends State<DetectCycleLinkedListPracticeQuiz> {
  int _activeSubTab = 0;
  ...
  final List<TestCaseData> _testCases = const [
    TestCaseData(inputStr: "1 -> 2 -> 3 -> 4 -> 2 (Cycle back to 2)", expectedOutputStr: "Has Cycle = TRUE"),
    TestCaseData(inputStr: "1 -> 2 -> 3 -> NULL (No Cycle)", expectedOutputStr: "Has Cycle = FALSE"),
    TestCaseData(inputStr: "1 -> 1 (Self Cycle)", expectedOutputStr: "Has Cycle = TRUE"),
    TestCaseData(inputStr: "NULL (Empty List)", expectedOutputStr: "Has Cycle = FALSE"),
  ];
  ...
  final List<QuizQuestion> _quizQuestions = const [
    QuizQuestion(
      questionEn: "1. Why are `slow` and `fast` pointers GUARANTEED to meet if a cycle exists in a linked list?",
      questionBn: "১. লিঙ্কড লিস্টে কোনো সাইকেল থাকলে কেন `slow` ও `fast` পয়েন্টার দুটি অবশ্যই এক সময় মিলবে?",
      optionsEn: [
        "A) Relative speed difference is 1 step per turn (2 - 1 = 1); inside a closed loop of length C, the gap shrinks by 1 until 0!",
        "B) Random probability",
        "C) Fast pointer jumps backwards",
        "D) They run in opposite directions"
      ],
      optionsBn: [
        "A) পয়েন্টার দুটির আপেক্ষিক বেগ প্রতি লুপে ১ নোড (2 - 1 = 1); তাই বন্ধ সাইকেলের ভেতর প্রতিটি পদক্ষেপে তাদের দূরত্ব ১ ঘর করে কমে শেষ পর্যন্ত ০ হয়!",
        "B) র্যান্ডম সুযোগের কারণে",
        "C) Fast পয়েন্টার পিছনের দিকে লাফ দেয়",
        "D) তারা বিপরীত দিকে দৌড়ায়"
      ],
      correctOptionIndex: 0,
      explanationEn: "Correct! Relative velocity inside the closed cycle loop is 2 - 1 = 1 step per iteration, so the distance between them MUST reduce to 0 within at most C steps.",
      explanationBn: "সঠিক! বদ্ধ সাইকেলের ভেতর তাদের আপেক্ষিক বেগ ২ - ১ = ১ নোড/ধাপ, ফলে দূরত্বের ব্যবধান প্রতি লুপে ১ ঘর করে কমে ০ তে পৌঁছাবেই।",
    ),
    QuizQuestion(
      questionEn: "2. What is the Space Complexity of Floyd's Cycle Detection Algorithm?",
      questionBn: "২. ফ্লয়েডের সাইকেল ডিটেকশন অ্যালগরিদমের Space Complexity কত?",
      optionsEn: ["A) O(1) Auxiliary Space", "B) O(N) Space (using Hash Set)", "C) O(N^2) Space", "D) O(log N) Space"],
      optionsBn: ["A) O(1) অতিরিক্ত স্পেস", "B) O(N) স্পেস (হ্যাশ সেট দিয়ে)", "C) O(N^2) স্পেস", "D) O(log N) স্পেস"],
      correctOptionIndex: 0,
      explanationEn: "Correct! Floyd's two-pointer algorithm uses only 2 pointer variables (`slow` and `fast`), achieving O(1) constant space without allocating a Hash Set.",
      explanationBn: "সঠিক! কোনো হ্যাশ সেট ব্যবহার না করে কেবল ২টি পয়েন্টার (`slow` ও `fast`) দিয়ে কাজ সম্পন্ন হওয়ায় এতে স্পেস লাগে O(1)।",
    ),
  ];
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Flexible(
                          child: Text(
                            widget.isEnglish ? "Cycle Detection Quiz" : "সাইকেল ডিটেকশন কুইজ",
                            overflow: TextOverflow.ellipsis,
                            maxLines: 1,
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(
                    child: Text(
                      widget.isEnglish ? "Cycle Detection Test Runner" : "সাইকেল ডিটেকশন টেস্ট রানার",
              ...
              Text(
                widget.isEnglish
                    ? "Verify cycle detection logic against cyclic linked lists, linear lists, self-cycles, and empty lists."
                    : "সাইক্লিক লিঙ্কড লিস্ট, লিনিয়ার লিস্ট, সেলফ-সাইকেল ও নাল লিস্ট কেসে ডিটেকশন লজিক টেস্ট করুন।",
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      widget.isEnglish ? "Cycle Detection Mastery Quiz" : "সাইকেল ডিটেকশন মাস্টার কুইজ",
