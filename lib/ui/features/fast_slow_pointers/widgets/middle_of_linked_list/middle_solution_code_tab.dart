...
class MiddleSolutionCodeTab extends StatefulWidget {
  final bool isEnglish;

  const MiddleSolutionCodeTab({
    super.key,
    required this.isEnglish,
  });

  @override
  State<MiddleSolutionCodeTab> createState() => _MiddleSolutionCodeTabState();
}

class _MiddleSolutionCodeTabState extends State<MiddleSolutionCodeTab> {
  final List<int> _currentNodes = [1, 2, 3, 4, 5];
  ...
  void _handleUserMove() {
    ...
    setState(() {
      if (_userFast < n && _userFast + 1 < n) {
        _userSlow += 1;
        _userFast += 2;

        if (_userFast >= n - 1) {
          _userSolved = true;
          _userFeedbackEn = "🎉 PERFECT! Fast reached end! Middle node is at index $_userSlow (value: ${_currentNodes[_userSlow]})!";
          _userFeedbackBn = "🎉 দারুণ! Fast লিস্টের শেষে পৌঁছেছে! মিডল নোড ইনডেক্স $_userSlow (মান: ${_currentNodes[_userSlow]})!";
        } else {
          _userFeedbackEn = "Step done: Slow is at idx $_userSlow (val: ${_currentNodes[_userSlow]}), Fast is at idx $_userFast (val: ${_currentNodes[_userFast]}). Continue!";
          _userFeedbackBn = "ধাপ সম্পন্ন: Slow ইনডেক্স $_userSlow এ, Fast ইনডেক্স $_userFast এ। পয়েন্টার চালান!";
        }
      } else {
        _userSolved = true;
        _userFeedbackEn = "🎉 Middle node found at index $_userSlow (value: ${_currentNodes[_userSlow]})!";
        _userFeedbackBn = "🎉 মিডল নোড প্রাপ্ত: ইনডেক্স $_userSlow (মান: ${_currentNodes[_userSlow]})!";
      }
    });
  }
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text(widget.isEnglish ? '🎮 Practice Mode: Solve It Yourself!' : '🎮 প্র্যাকটিস মোড: নিজে ট্রাই করুন!', style: TextStyle(fontSize: Responsive.sp(context, 16), fontWeight: FontWeight.bold, color: Colors.white)),
                    ],
                  ),
                  const SizedBox(height: 12),
                  Text(widget.isEnglish ? 'Current List: $_currentNodes' : 'বর্তমান লিস্ট: $_currentNodes', style: TextStyle(color: AppTheme.accentNeonCyan, fontWeight: FontWeight.bold, fontSize: Responsive.sp(context, 13))),
            ...
            // Pure Multi-Language Solution Code Section
            Container(
              padding: EdgeInsets.all(Responsive.sp(context, 18)),
              decoration: BoxDecoration(
                color: AppTheme.surfaceDark,
...
public:
    ListNode* middleNode(ListNode* head) {
        ...
        return slow; // Points to middle node (2nd middle for even length)
    }
};""";
    } else if (lang == "Java") {
      code = """
