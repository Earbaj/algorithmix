...
class ValidateBstPracticeQuiz extends StatefulWidget {
  final bool isEnglish;

  const ValidateBstPracticeQuiz({
    super.key,
    required this.isEnglish,
  });

  @override
  State<ValidateBstPracticeQuiz> createState() =>
      _ValidateBstPracticeQuizState();
}
...
class _ValidateBstPracticeQuizState
    extends State<ValidateBstPracticeQuiz> {
  int _activeSubTab = 0;
  ...
  final List<TestCaseData> _testCases = const [
    TestCaseData(inputStr: "root = [2, 1, 3]", expectedOutputStr: "True"),
    TestCaseData(inputStr: "root = [5, 1, 4, null, null, 3, 6]", expectedOutputStr: "False"),
    TestCaseData(inputStr: "root = [10, 5, 15, null, null, 6, 20]", expectedOutputStr: "False (node 6 in right subtree < root 10)"),
  ];
  ...
  final List<QuizQuestion> _quizQuestions = const [
    QuizQuestion(
      questionEn: "1. Why is checking only local parent-child relationship (`node.left < node` and `node.right > node`) WRONG for validating a BST?",
      questionBn: "১. কেবল নোডের স্থানীয় চাইল্ড তুলনা করা (`node.left < node`) কেন BST ভ্যালিডেশনের জন্য ভুল?",
      optionsEn: [
        "A) Because ALL nodes in the right subtree must be strictly greater than EVERY ancestor root node up the tree!",
        "B) Because child nodes can be null",
        "C) Because trees have no parent pointers",
        "D) It is actually correct"
      ],
      optionsBn: [
        "A) কারণ ডান সাবট্রির সমস্ত মানকে উপরের সমস্ত অভিভাবক রুটের মানের চেয়ে বড় হতে হয়!",
        "B) কারণ চাইল্ড নোড নাল হতে পারে",
        "C) কারণ ট্রিতে প্যারেন্ট পয়েন্টার থাকে না",
        "D) এটি আসলে সঠিক পদ্ধতি"
      ],
      correctOptionIndex: 0,
      explanationEn: "Correct! In `root = 10` with right child `15` having a left child `6`, local `6 < 15` passes, but global `6 < 10` violates the BST invariant!",
      explanationBn: "সঠিক! রুট ১০ এর ডান চাইল্ড ১৫ এবং ১৫ এর বামে ৬ থাকলে, স্থানীয়ভাবে ৬ < ১৫ ঠিক কিন্তু রুটের ক্ষেত্রে ৬ < ১০ ভুল প্রমাণিত হয়।",
    ),
    QuizQuestion(
      questionEn: "2. How does the Range Bound `validate(node, minBound, maxBound)` approach pass constraints down the recursion tree?",
      questionBn: "২. রেঞ্জ বাউন্ড `validate(node, minBound, maxBound)` পদ্ধতি কীভাবে রিকার্সন ট্রিতে সীমানা পাস করে?",
      optionsEn: [
        "A) Left child inherits `(minBound, node.val)`, right child inherits `(node.val, maxBound)`!",
        "B) It adds 1 to min and max",
        "C) It resets bounds to zero",
        "D) Bounds double at each level"
      ],
      optionsBn: [
        "A) বাম চাইল্ড `(minBound, node.val)` পায়, ডান চাইল্ড `(node.val, maxBound)` সীমানা পায়!",
        "B) এটি min ও max এ ১ যোগ করে",
        "C) এটি সীমানা শূন্যে রিসেট করে",
        "D) প্রতিটি লেভেলে সীমানা দ্বিগুণ হয়"
      ],
      correctOptionIndex: 0,
      explanationEn: "Correct! Branching left tightens the upper bound (`max = node.val`), while branching right tightens the lower bound (`min = node.val`).",
      explanationBn: "সঠিক! বামে গেলে ম্যাক্স সীমানা চেপে আসে এবং ডানে গেলে মিন সীমানা চেপে আসে।",
    ),
  ];
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Flexible(
                          child: Text(
                            widget.isEnglish ? "Validate BST Quiz" : "ভ্যালিডেট BST কুইজ",
                            overflow: TextOverflow.ellipsis,
                            maxLines: 1,
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(
                    child: Text(
                      widget.isEnglish ? "Validate BST Test Runner" : "ভ্যালিডেট BST টেস্ট রানার",
              ...
              Text(
                widget.isEnglish
                    ? "Verify BST range invariant logic for valid and invalid binary tree structures."
                    : "ভ্যালিড ও ইনভ্যালিড ট্রির ক্ষেত্রে BST সীমানা ভ্যালিডেশন টেস্ট করুন।",
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      widget.isEnglish ? "Validate BST Mastery Quiz" : "ভ্যালিডেট BST মাস্টার কুইজ",
