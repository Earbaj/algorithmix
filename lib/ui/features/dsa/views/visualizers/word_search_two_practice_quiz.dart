...
class WordSearchTwoPracticeQuiz extends StatefulWidget {
  final bool isEnglish;

  const WordSearchTwoPracticeQuiz({
    super.key,
    required this.isEnglish,
  });

  @override
  State<WordSearchTwoPracticeQuiz> createState() =>
      _WordSearchTwoPracticeQuizState();
}
...
class _WordSearchTwoPracticeQuizState
    extends State<WordSearchTwoPracticeQuiz> {
  int _activeSubTab = 0;
  ...
  final List<TestCaseData> _testCases = const [
    TestCaseData(inputStr: "board = [[\"o\",\"a\",\"a\",\"n\"],[\"e\",\"t\",\"a\",\"e\"],[\"i\",\"h\",\"k\",\"r\"],[\"i\",\"f\",\"l\",\"v\"]], words = [\"oath\",\"pea\",\"eat\",\"rain\"]", expectedOutputStr: "[\"oath\", \"eat\"]"),
    TestCaseData(inputStr: "board = [[\"a\",\"b\"],[\"c\",\"d\"]], words = [\"abcb\"]", expectedOutputStr: "[]"),
  ];
  ...
  final List<QuizQuestion> _quizQuestions = const [
    QuizQuestion(
      questionEn: "1. Why does using a Trie dramatically speed up 2D Grid Word Search II compared to standard DFS on each word independently?",
      questionBn: "১. প্রতিটি শব্দের জন্য পৃথক DFS চালানোর চেয়ে ট্রাই ব্যবহার করে Word Search II তে কেন পারফরম্যান্স অবিশ্বাস্যভাবে বেড়ে যায়?",
      optionsEn: [
        "A) Trie allows sharing common prefix paths across ALL dictionary words simultaneously, allowing DFS to PRUNE invalid grid search paths immediately!",
        "B) Trie sorts the 2D grid matrix",
        "C) Trie reduces grid dimensions",
        "D) Trie avoids grid visited checks"
      ],
      optionsBn: [
        "A) ট্রাই একই সাথে সমস্ত ডিকশনারি শব্দের সাধারণ প্রিফিক্স পাথ শেয়ার করে, যার ফলে DFS কোনো প্রিফিক্স ট্রাইতে না থাকলে সাথে সাথে সার্চ প্রুন (Prune) করে ফেলতে পারে!",
        "B) ট্রাই ২D গ্রিড ম্যাট্রিক্স সর্ট করে",
        "C) ট্রাই গ্রিড ডাইমেনশন কমায়",
        "D) ট্রাই গ্রিড ভিজিটেড চেক বাদ দেয়"
      ],
      correctOptionIndex: 0,
      explanationEn: "Correct! Pruning invalid prefix branches early eliminates millions of redundant 4-directional grid DFS recursion calls.",
      explanationBn: "সঠিক! ট্রাই প্রিফিক্স মিসিং হলে সাথে সাথে ব্যাকট্র্যাক করায় লক্ষ লক্ষ অপ্রয়োজনীয় DFS ব্যাকট্র্যাকিং বেঁচে যায়।",
    ),
    QuizQuestion(
      questionEn: "2. How do we prevent duplicate word matches in Word Search II when the same word is found through multiple grid paths?",
      questionBn: "২. একই শব্দ গ্রিডের ভিন্ন ভিন্ন পথ দিয়ে পাওয়া গেলে দ্বিগুণ কাউন্টিং বা ডুপ্লিকেট শব্দ এড়াতে আমরা কী করি?",
      optionsEn: [
        "A) Clear node->word = \"\" immediately after finding a word in Trie node!",
        "B) Delete the grid",
        "C) Reset root to null",
        "D) Use 1D array"
      ],
      optionsBn: [
        "A) ট্রাই নোডে শব্দ পাওয়ার সাথে সাথেই `node->word = \"\"` (ফাঁকা) করে দেওয়া!",
        "B) গ্রিড মুছে দেওয়া",
        "C) রুট পয়েন্টার নাল করা",
        "D) ১D অ্যারে ব্যবহার করা"
      ],
      correctOptionIndex: 0,
      explanationEn: "Correct! Clearing `node->word` or storing a set guarantees unique result entries.",
      explanationBn: "সঠিক! `node->word` ফাঁকা করে দিলে সেই শব্দ ভবিষ্যতে দ্বিতীয়বার অন্য পথ থেকে পাওয়া গেলেও রেজাল্টে আর যোগ হবে না।",
    ),
  ];
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Flexible(
                          child: Text(
                            widget.isEnglish ? "Word Search II Quiz" : "ওয়ার্ড সার্চ ২ কুইজ",
                            overflow: TextOverflow.ellipsis,
                            maxLines: 1,
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(
                    child: Text(
                      widget.isEnglish ? "Word Search II Test Runner" : "ওয়ার্ড সার্চ ২ টেস্ট রানার",
              ...
              Text(
                widget.isEnglish
                    ? "Verify grid backtracking + Trie prefix pruning for multi-word search."
                    : "২D গ্রিড ব্যাকট্র্যাকিং ও ট্রাই প্রিফিক্স প্রুনিং লজিক ভেরিফাই করুন।",
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      widget.isEnglish ? "Word Search II Mastery Quiz" : "ওয়ার্ড সার্চ ২ কুইজ",
