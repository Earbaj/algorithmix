...
import 'package:algorithmix/domain/models/fast_slow_pointers_data.dart';
import 'package:algorithmix/ui/core/theme/app_theme.dart';
import 'package:algorithmix/ui/core/utils/responsive.dart';
import '../widgets/fast_slow_pointers_visualizer.dart';

class FastSlowPointersDetailScreen extends StatefulWidget {
  const FastSlowPointersDetailScreen({super.key});

  @override
  State<FastSlowPointersDetailScreen> createState() => _FastSlowPointersDetailScreenState();
}

class _FastSlowPointersDetailScreenState extends State<FastSlowPointersDetailScreen>
  ...
  Widget build(BuildContext context) {
    final intro = FastSlowPointersData.getConceptIntro(_isEnglish);

    return Scaffold(
      backgroundColor: AppTheme.primaryDark,
      appBar: AppBar(
        title: Text(_isEnglish ? 'Fast & Slow Pointers Deep Dive' : 'ফাস্ট ও স্লো পয়েন্টারস গাইড'),
        centerTitle: true,
        actions: [
          Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            // 3 Main Fast & Slow Pointer Types
            Text(
              intro["typesTitle"]!,
              style: TextStyle(
            ...
            const SizedBox(height: 12),
            _buildCodeBox("Template 1: Floyd's Cycle Detection (Linked List Cycle)", """
bool hasCycle(ListNode *head) {
    ...
    return false;
            ...
            _buildCodeBox("Template 2: Middle of the Linked List", """
ListNode* middleNode(ListNode* head) {
    ...
    return slow; // Middle Node!
            ...
            const SizedBox(height: 16),
            _buildCodeBox("Template 3: Find Cycle Entry Node (LeetCode 142)", """
ListNode *detectCycle(ListNode *head) {
  ...
  Widget _buildVisualizerTab() {
    ...
    return ResponsiveCenter(
      padding: EdgeInsets.all(hPadding),
      child: SingleChildScrollView(
        child: FastSlowPointersVisualizer(isEnglish: _isEnglish),
      ),
    );
  }
  ...
  Widget _buildProblemsTab() {
    final hPadding = Responsive.horizontalPadding(context);
    final easy = FastSlowPointersData.getEasyProblems();
    final medium = FastSlowPointersData.getMediumProblems();
    final hard = FastSlowPointersData.getHardProblems();
  ...
  Widget _buildProblemSection(String title, List<FastSlowPointersProblem> problems, Color diffColor) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
  ...
  Widget _buildProblemCard(FastSlowPointersProblem p, Color diffColor) {
    return Container(
  ...
  Widget _buildMistakesTab() {
    final hPadding = Responsive.horizontalPadding(context);
    final mistakes = FastSlowPointersData.getCommonMistakes(_isEnglish);
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _buildRoadmapStep(1, _isEnglish ? "Master 1-Step vs 2-Step Pointer Mechanics" : "১-ধাপ বনাম ২-ধাপ পয়েন্টার মেকানিক্স আয়ত্তকরণ", _isEnglish ? "Understand O(1) space traversal speeds." : "O(1) স্পেসের স্পিড রেসিও বুঝুন।"),
            _buildRoadmapStep(2, _isEnglish ? "Master Cycle Detection & Collision Proof" : "সাইকেল ডিটেকশন ও কোলাইশন প্রুফ", _isEnglish ? "Detect loop intersection using slow == fast." : "slow == fast দিয়ে ইন্টারসেকশন শনাক্ত করুন।"),
            _buildRoadmapStep(3, _isEnglish ? "Master Cycle Entry Node Discovery" : "সাইকেল শুরুর নোড (Cycle Entry Node) নির্ণয়", _isEnglish ? "Reset slow = head and advance both pointers 1 step to meet at entry." : "slow=head করে ১ ধাপ করে এগিয়ে শুরুর নোড পান।"),
            _buildRoadmapStep(4, _isEnglish ? "Master Single-Pass Middle Node Finding" : "এক পাসে মিডল নোড ও লিস্ট পার্টনিং", _isEnglish ? "Find exact middle node to reverse list and check palindromes." : "মিডল নোড দিয়ে লিস্ট রিভার্স ও প্যালিন্ড্রোম চেক করুন।"),
            _buildRoadmapStep(5, _isEnglish ? "Master Floyd's Algorithm on Arrays" : "অ্যারেতে ফ্লয়েডের অ্যালগরিদম প্রয়োগ (LeetCode 287)", _isEnglish ? "Treat array elements as pointers to solve duplicate number problems." : "অ্যারে উপাদানকে লিঙ্কড পয়েন্টার হিসেবে ব্যবহার করুন।"),
            const SizedBox(height: 24),
          ],
        ),
      ),
