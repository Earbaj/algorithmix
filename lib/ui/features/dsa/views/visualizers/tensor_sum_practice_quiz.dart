...
class TensorSumPracticeQuiz extends StatefulWidget {
  final bool isEnglish;

  const TensorSumPracticeQuiz({
    super.key,
    required this.isEnglish,
  });

  @override
  State<TensorSumPracticeQuiz> createState() => _TensorSumPracticeQuizState();
}
...
class _TensorSumPracticeQuizState extends State<TensorSumPracticeQuiz> {
  int _activeSubTab = 0;
  ...
  final List<TestCaseData> _testCases = const [
    TestCaseData(inputStr: "tensor = [[[1,2],[3,4]], [[5,6],[7,8]]] (2x2x2)", expectedOutputStr: "Total Sum = 36"),
    TestCaseData(inputStr: "tensor = [[[10]]] (1x1x1)", expectedOutputStr: "Total Sum = 10"),
    TestCaseData(inputStr: "tensor = [[[-5, -10], [5, 10]]]", expectedOutputStr: "Total Sum = 0"),
    TestCaseData(inputStr: "tensor = [[[1, 1], [1, 1]], [[2, 2], [2, 2]]]", expectedOutputStr: "Total Sum = 12"),
  ];
  ...
  final List<QuizQuestion> _quizQuestions = const [
    QuizQuestion(
      questionEn: "1. What is the Time Complexity of summing all elements in a 3D Tensor of size D x R x C?",
      questionBn: "১. D x R x C আকারের ৩D টেনসরের সব উপাদানের যোগফল বের করার Time Complexity কত?",
      optionsEn: ["A) O(D * R * C)", "B) O(D + R + C)", "C) O(D^3)", "D) O(R * C)"],
      optionsBn: ["A) O(D * R * C)", "B) O(D + R + C)", "C) O(D^3)", "D) O(R * C)"],
      correctOptionIndex: 0,
      explanationEn: "Correct! To sum all elements in a 3D volume, 3 nested loops iterate through all D * R * C elements exactly once.",
      explanationBn: "সঠিক! ৩D ভলিউমের প্রতিটি উপাদান যোগ করতে ৩টি নেস্টেড লুপ সর্বমোট D * R * C টি উপাদানের ওপর কাজ করে।",
    ),
    QuizQuestion(
      questionEn: "2. How are 3D Tensor indices mapped when flattened into a single 1D Array in memory?",
      questionBn: "২. ৩D টেনসরের মেমোরি ফ্ল্যাটেনিংয়ে tensor[d][r][c] মানটি ১D অ্যারের কোন ইন্ডেক্সে থাকে?",
      optionsEn: [
        "A) index = d * (R * C) + r * C + c",
        "B) index = d + r + c",
        "C) index = (d * R * C)^2",
      ...
      correctOptionIndex: 0,
      explanationEn: "Correct! Flattened 3D index formula is d * (R * C) + r * C + c, which is heavily used in GPU tensor engines!",
      explanationBn: "সঠিক! ১D অ্যারেতে ৩D ইন্ডেক্স রূপান্তরের সূত্র: d * (R * C) + r * C + c। জিপিউ ও ডিপ লার্নিং ইঞ্জিনে এই সূত্র ব্যবহৃত হয়!",
    ),
    QuizQuestion(
      questionEn: "3. What is the Auxiliary Space Complexity of computing 3D Tensor sum?",
      questionBn: "৩. ৩D টেনসর সামের Auxiliary Space Complexity কত?",
      optionsEn: ["A) O(1) Constant Space", "B) O(D * R * C) Space", "C) O(D) Space", "D) O(R * C) Space"],
      optionsBn: ["A) O(1) কনস্ট্যান্ট স্পেস", "B) O(D * R * C) স্পেস", "C) O(D) স্পেস", "D) O(R * C) স্পেস"],
      correctOptionIndex: 0,
      explanationEn: "Correct! Only a single accumulator variable 'total' is updated during traversal, taking O(1) auxiliary space.",
      explanationBn: "সঠিক! কেবল একটি সিঙ্গেল ভ্যারিয়েবল 'total' দিয়ে সব উপাদান যোগ করা যায়, ফলে স্পেস লাগে O(1)।",
    ),
  ];
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Flexible(
                          child: Text(
                            widget.isEnglish ? "3D Tensor Quiz" : "৩D টেনসর কুইজ",
                            overflow: TextOverflow.ellipsis,
                            maxLines: 1,
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(
                    child: Text(
                      widget.isEnglish ? "3D Tensor Sum Test Runner" : "৩D টেনসর সাম টেস্ট রানার",
              ...
              Text(
                widget.isEnglish
                    ? "Verify 3D volume summation logic against various tensor depth and dimension sizes."
                    : "বিভিন্ন টেনসর ডেপথ ও মাত্রা সাইজে ৩D সামেশন লজিক টেস্ট ও ভেরিফাই করুন।",
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      widget.isEnglish ? "3D Tensor Sum Mastery Quiz" : "৩D টেনসর সাম মাস্টার কুইজ",
