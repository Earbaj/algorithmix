...
class ReverseDoublyLinkedListPracticeQuiz extends StatefulWidget {
  final bool isEnglish;

  const ReverseDoublyLinkedListPracticeQuiz({
    super.key,
    required this.isEnglish,
  });

  @override
  State<ReverseDoublyLinkedListPracticeQuiz> createState() =>
      _ReverseDoublyLinkedListPracticeQuizState();
}
...
class _ReverseDoublyLinkedListPracticeQuizState
    extends State<ReverseDoublyLinkedListPracticeQuiz> {
  int _activeSubTab = 0;
  ...
  final List<TestCaseData> _testCases = const [
    ...
    TestCaseData(inputStr: "5 (Single Node)", expectedOutputStr: "5"),
  ];
  ...
  final List<QuizQuestion> _quizQuestions = const [
    QuizQuestion(
      questionEn: "1. What makes reversing a Doubly Linked List different from a Singly Linked List?",
      questionBn: "১. Doubly Linked List রিভার্স করা Singly Linked List এর চেয়ে আলাদা কেন?",
      optionsEn: [
        "A) DLL nodes contain TWO pointer references (`prev` and `next`); BOTH must be swapped!",
        "B) DLL nodes cannot be reversed",
        "C) DLL requires O(N^2) space",
        "D) DLL only moves forward"
      ],
      optionsBn: [
        "A) DLL এর নোডে ২টি করে পয়েন্টার রেফারেন্স (`prev` ও `next`) থাকে; দুটিই Swap করতে হয়!",
        "B) DLL নোড রিভার্স করা অসম্ভব",
        "C) DLL এ O(N^2) স্পেস লাগে",
        "D) DLL কেবল সামনের দিকে চলে"
      ],
      correctOptionIndex: 0,
      explanationEn: "Correct! Every node in a Doubly Linked List has both `prev` and `next` links. Swapping both links reverses the direction of the entire chain.",
      explanationBn: "সঠিক! Doubly Linked List এর প্রতিটি নোডে prev ও next উভয় পয়েন্টার থাকায় দুটি লিংকই সোয়াপ করতে হয়।",
    ),
    QuizQuestion(
      questionEn: "2. Why do we advance `curr = curr->prev` after swapping `curr->prev` and `curr->next`?",
      questionBn: "২. `curr->prev` এবং `curr->next` সোয়াপ করার পর কেন `curr = curr->prev` লিখে এগোতে হয়?",
      optionsEn: [
        "A) Because swapping placed the original forward node into the `prev` pointer slot!",
        "B) It is a syntax bug",
        "C) To go backwards",
        "D) To delete the node"
      ],
      optionsBn: [
        "A) কারণ পয়েন্টার অদলবদল হওয়ায় আসল পরবর্তী নোডের এড্রেসটি এখন `prev` পয়েন্টারের ভেতর চলে গেছে!",
        "B) এটি একটি সিনট্যাক্স এরর",
        "C) পিছনের দিকে যেতে",
        "D) নোড ডিলেট করতে"
      ],
      correctOptionIndex: 0,
      explanationEn: "Correct! Since `curr->prev` was swapped with `curr->next`, the original next node is now reached via `curr->prev`!",
      explanationBn: "সঠিক! সোয়াপিং এর পর মূল পরবর্তী নোডের পয়েন্টারটি `curr->prev` ফিল্ডে অবস্থান করে।",
    ),
  ];
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Flexible(
                          child: Text(
                            widget.isEnglish ? "DLL Reversal Quiz" : "DLL রিভার্সাল কুইজ",
                            overflow: TextOverflow.ellipsis,
                            maxLines: 1,
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(
                    child: Text(
                      widget.isEnglish ? "Reverse Doubly Linked List Test Runner" : "DLL রিভার্সাল টেস্ট রানার",
              ...
              Text(
                widget.isEnglish
                    ? "Verify bidirectional pointer swap logic against multi-node, 2-node, and single node DLLs."
                    : "বহু-নোড ও সিঙ্গেল নোডের দ্বিমুখী পয়েন্টার সোয়াপ লজিক ভেরিফাই করুন।",
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      widget.isEnglish ? "DLL Reversal Mastery Quiz" : "DLL রিভার্সাল মাস্টার কুইজ",
