...
class SlidingWindowMaxPracticeQuiz extends StatefulWidget {
  final bool isEnglish;

  const SlidingWindowMaxPracticeQuiz({
    super.key,
    required this.isEnglish,
  });

  @override
  State<SlidingWindowMaxPracticeQuiz> createState() =>
      _SlidingWindowMaxPracticeQuizState();
}
...
class _SlidingWindowMaxPracticeQuizState
    extends State<SlidingWindowMaxPracticeQuiz> {
  int _activeSubTab = 0;
  ...
  final List<TestCaseData> _testCases = const [
    TestCaseData(inputStr: "nums = [1,3,-1,-3,5,3,6,7], k = 3", expectedOutputStr: "res = [3,3,5,5,6,7]"),
    TestCaseData(inputStr: "nums = [1], k = 1", expectedOutputStr: "res = [1]"),
    TestCaseData(inputStr: "nums = [9,11], k = 2", expectedOutputStr: "res = [11]"),
  ];
  ...
  final List<QuizQuestion> _quizQuestions = const [
    QuizQuestion(
      questionEn: "1. Why does a Monotonic Deque achieve O(N) linear time for Sliding Window Maximum?",
      questionBn: "১. মনোটোনিক Deque কেন স্লাইডিং উইন্ডো ম্যাক্সিমামের জন্য O(N) লিনিয়ার সময় অর্জন করে?",
      optionsEn: [
        "A) Every index is pushed and popped from the Deque at most ONCE, giving Amortized O(1) per step across N elements!",
        "B) It sorts each window in O(k log k)",
        "C) It scans the entire array for each window",
        "D) It uses heap priority queue in O(N log K)"
      ],
      optionsBn: [
        "A) প্রতিটি ইনডেক্স Deque এ সর্বাধিক একবার পুশ এবং একবার পপ হয়, যার ফলে N উপাদানের জন্য গড়ে Amortized O(1) সময় লাগে!",
        "B) এটি প্রতিটি উইন্ডো O(k log k) এ সোর্ট করে",
        "C) এটি প্রতিটি উইন্ডোর জন্য পুরো অ্যারে স্ক্যান করে",
        "D) এটি O(N log K) এ হিপ ব্যবহার করে"
      ],
      correctOptionIndex: 0,
      explanationEn: "Correct! Even though a single element might trigger multiple pops from the back, overall each element enters and leaves the deque at most once.",
      explanationBn: "সঠিক! একটি বড় মান পাওয়ার পর একাধিক ছোট ইনডেক্স পপ হলেও, পুরো প্রসেসে প্রতিটি ইনডেক্স ১ বারের বেশি পুশ/পপ হয় না।",
    ),
    QuizQuestion(
      questionEn: "2. Which index is popped from `dq.front()` when the window slides forward past its range?",
      questionBn: "২. উইন্ডো সামনের দিকে এগিয়ে গেলে `dq.front()` থেকে কোন ইনডেক্সটি পপ করা হয়?",
      optionsEn: [
        "A) Out-of-window index `i - k`!",
        "B) The smallest element index",
        "C) Index 0 always",
        "D) The maximum index"
      ],
      optionsBn: [
        "A) উইন্ডোর সীমানার বাইরের ইনডেক্স `i - k`!",
        "B) সবচেয়ে ছোট মানের ইনডেক্স",
        "C) সর্বদা ০ ইনডেক্স",
        "D) সর্বাধিক মানের ইনডেক্স"
      ],
      correctOptionIndex: 0,
      explanationEn: "Correct! If `dq.front() == i - k`, it means the front element index is no longer inside the current sliding window of size `k`, so it is popped from the front.",
      explanationBn: "সঠিক! `dq.front() == i - k` হলে ফ্রন্টের ইনডেক্সটি উইন্ডোর পেছনে পড়ে গেছে, তাই ফ্রন্ট থেকে ডিকিউ করা হয়।",
    ),
  ];
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Flexible(
                          child: Text(
                            widget.isEnglish ? "Monotonic Deque Quiz" : "মনোটোনিক Deque কুইজ",
                            overflow: TextOverflow.ellipsis,
                            maxLines: 1,
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(
                    child: Text(
                      widget.isEnglish ? "Sliding Window Max Test Runner" : "স্লাইডিং উইন্ডো টেস্ট রানার",
              ...
              Text(
                widget.isEnglish
                    ? "Verify monotonic deque sliding window max calculation across multiple window sizes and array lengths."
                    : "মনোটোনিক Deque স্লাইডিং উইন্ডো ম্যাক্সিমাম অ্যালগরিদম ভেরিফাই করুন।",
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      widget.isEnglish ? "Monotonic Deque Mastery Quiz" : "মনোটোনিক Deque মাস্টার কুইজ",
