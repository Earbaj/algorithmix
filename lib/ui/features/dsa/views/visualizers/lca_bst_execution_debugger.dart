...
class LcaBstExecutionDebugger extends StatefulWidget {
  final bool isEnglish;

  const LcaBstExecutionDebugger({
    super.key,
    required this.isEnglish,
  });

  @override
  State<LcaBstExecutionDebugger> createState() =>
      _LcaBstExecutionDebuggerState();
}
...
class _LcaBstExecutionDebuggerState extends State<LcaBstExecutionDebugger> {
  ...
  void initState() {
    ...
    _steps = const [
      DebuggerStepData(
        ...
        pVal: 2,
        qVal: 8,
        explanationEn: "Line 1: Entry into lowestCommonAncestor(root=6, p=2, q=8).",
        explanationBn: "লাইন ১: lowestCommonAncestor(root=6, p=2, q=8) এ প্রবেশ।",
      ),
      DebuggerStepData(
        activeLineIndex: 5,
        rootVal: 6,
        pVal: 2,
        qVal: 8,
        explanationEn: "Line 6: `p=2 < 6` AND `q=8 > 6` -> Split condition met! Return root Node 6 as LCA! 🎉",
        explanationBn: "লাইন ৬: `p=2 < 6` ও `q=8 > 6` -> দুটি মান আলাদা দিকে ভাগ হয়ে গেছে! রুট নোড 6 রিটার্ন (LCA)! 🎉",
      ),
    ];
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text(
          widget.isEnglish
              ? "Line-by-Line Execution Debugger & LCA Inspector"
        ...
        children: [
          Row(
            children: [
              ...
              Text(
                widget.isEnglish ? "LCA Method Watcher" : "LCA মেথড ওয়াচার",
          ...
          Wrap(
            ...
            runSpacing: 10,
            children: [
              _buildVariableBadge("root->val", "${step.rootVal}", AppTheme.accentNeonCyan),
              _buildVariableBadge("p->val", "${step.pVal}", AppTheme.accentGreen),
              _buildVariableBadge("q->val", "${step.qVal}", AppTheme.accentAmber),
            ],
          ),
        ],
      ),
