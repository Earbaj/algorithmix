...
class TopKElementsConceptTabView extends StatelessWidget {
  final Map<String, String> intro;
  final bool isEnglish;

  const TopKElementsConceptTabView({
    super.key,
    required this.intro,
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            // 3 Main Top K Elements Patterns
            Text(
              intro["typesTitle"]!,
              style: TextStyle(
            ...
            const SizedBox(height: 12),
            _buildCodeBox("Template 1: Kth Largest Element (Min-Heap of Size K)", """
int findKthLargest(vector<int>& nums, int k) {
    ...
    return minHeap.top(); // Kth largest element found!
}"""),
            const SizedBox(height: 16),
            _buildCodeBox("Template 2: Top K Frequent Elements (LeetCode 347)", """
vector<int> topKFrequent(vector<int>& nums, int k) {
    ...
    vector<int> res; while (!minHeap.empty()) { res.push_back(minHeap.top().second); minHeap.pop(); }
    return res;
}"""),
            const SizedBox(height: 16),
            _buildCodeBox("Template 3: K Closest Points to Origin (LeetCode 973)", """
vector<vector<int>> kClosest(vector<vector<int>>& points, int k) {
    ...
    vector<vector<int>> res; while (!maxHeap.empty()) { res.push_back(points[maxHeap.top().second]); maxHeap.pop(); }
    return res;
}"""),
            const SizedBox(height: 24),
          ],
