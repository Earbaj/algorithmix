...
import 'package:algorithmix/domain/models/union_find_data.dart';
import 'package:algorithmix/ui/core/theme/app_theme.dart';
import 'package:algorithmix/ui/core/utils/responsive.dart';
import '../widgets/union_find_visualizer.dart';

class UnionFindDetailScreen extends StatefulWidget {
  const UnionFindDetailScreen({super.key});

  @override
  State<UnionFindDetailScreen> createState() => _UnionFindDetailScreenState();
}

class _UnionFindDetailScreenState extends State<UnionFindDetailScreen>
  ...
  Widget build(BuildContext context) {
    final intro = UnionFindData.getConceptIntro(_isEnglish);

    return Scaffold(
      backgroundColor: AppTheme.primaryDark,
      appBar: AppBar(
        title: Text(_isEnglish ? 'Union Find (Disjoint Set) Deep Dive' : 'ইউনিয়ন ফাইন্ড (DSU) গাইড'),
        centerTitle: true,
        actions: [
          Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _buildCodeBox("Template 1: Connected Components DSU Class (C++ LeetCode 547)", """
class DSU {
...
public:
    ...
    bool unite(int u, int v) {
        ...
        parent[rootV] = rootU; count--; return true; // Merge roots & decrement count
            ...
            _buildCodeBox("Template 2: Redundant Connection Cycle Detection (C++ LeetCode 684)", """
vector<int> findRedundantConnection(vector<vector<int>>& edges) {
    ...
    return {};
            ...
            const SizedBox(height: 16),
            _buildCodeBox("Template 3: Most Stones Removed (C++ LeetCode 947)", """
int removeStones(vector<vector<int>>& stones) {
  ...
  Widget _buildVisualizerTab() {
    ...
    return ResponsiveCenter(
      padding: EdgeInsets.all(hPadding),
      child: SingleChildScrollView(
        child: UnionFindVisualizer(isEnglish: _isEnglish),
      ),
    );
  }
  ...
  Widget _buildProblemsTab() {
    final hPadding = Responsive.horizontalPadding(context);
    final easy = UnionFindData.getEasyProblems();
    final medium = UnionFindData.getMediumProblems();
    final hard = UnionFindData.getHardProblems();
  ...
  Widget _buildProblemSection(String title, List<UnionFindProblem> problems, Color diffColor) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
  ...
  Widget _buildProblemCard(UnionFindProblem p, Color diffColor) {
    return Container(
  ...
  Widget _buildMistakesTab() {
    final hPadding = Responsive.horizontalPadding(context);
    final mistakes = UnionFindData.getCommonMistakes(_isEnglish);
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _buildRoadmapStep(1, _isEnglish ? "Master Path Compression & Union by Rank" : "পাথ কম্প্রেশন ও র‍্যাংক মার্জ", _isEnglish ? "Implement find(i) with parent[i] = find(parent[i]) assignment." : "`parent[i] = find(parent[i])` অ্যাসাইনমেন্ট দিয়ে O(α(N)) নিশ্চিত করুন।"),
            _buildRoadmapStep(2, _isEnglish ? "Master Connected Components Count" : "কানেক্টেড কম্পোনেন্ট গণনা (LeetCode 547)", _isEnglish ? "Initialize count = N and decrement on successful unite." : "`count = N` দিয়ে শুরু করে মার্জে কমান।"),
            _buildRoadmapStep(3, _isEnglish ? "Master Undirected Graph Cycle Detection" : "আনডিরেক্টেড গ্রাফ সাইকেল ডিটেকশন (LeetCode 684)", _isEnglish ? "Detect cycle edge when find(u) == find(v) occurs." : "`find(u) == find(v)` হলেই অপ্রয়োজনীয় এজ চিহ্নিত করুন।"),
            _buildRoadmapStep(4, _isEnglish ? "Master 2D Grid to 1D Index Mapping DSU" : "২D ম্যাট্রিক্স ১D ইনডেক্স DSU (LeetCode 305)", _isEnglish ? "Map cell (r, c) to r * C + c ID and unite adjacent land cells." : "ইনডেক্স ১D তে রূপান্তরিত করে দ্বীপ মার্জ করুন।"),
            _buildRoadmapStep(5, _isEnglish ? "Master Accounts Merge & Kruskal's MST" : "ইমেইল একাউন্ট মার্জিং ও Kruskal's MST", _isEnglish ? "Solve Accounts Merge (LeetCode 721) and Kruskal's Minimum Spanning Tree." : "Accounts Merge ও MST প্রবলেম সমাধান করুন।"),
            const SizedBox(height: 24),
          ],
        ),
      ),
