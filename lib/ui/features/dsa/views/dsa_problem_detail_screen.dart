...
class _DsaProblemDetailScreenState extends State<DsaProblemDetailScreen>
  ...
  // GRAPH (ADJACENCY LIST & MATRIX) CODE LINES & STEPS
  final List<String> _gr1CodeLines = [
  ...
  final List<DebugArrayStep> _gr1Steps = const [
    DebugArrayStep(
      activeLineIndex: 2,
      queueItems: ["0"],
      array1D: [0],
  ...
  final List<String> _gr2CodeLines = [
  ...
  final List<DebugArrayStep> _gr2Steps = const [
    DebugArrayStep(
      activeLineIndex: 1,
      stackItems: ["0"],
      array1D: [0],
  ...
  final List<String> _gr3CodeLines = [
  ...
  final List<DebugArrayStep> _gr3Steps = const [
    DebugArrayStep(
      activeLineIndex: 4,
      matrix2D: [[1, 1, 0], [1, 1, 0], [0, 0, 1]],
      minVal: 1,
      explanationEn: "Line 5: Encountered Land '1' at (0, 0) -> Count = 1. Trigger sink() DFS.",
      explanationBn: "লাইন ৫: গ্রিড (0, 0) এ মাটি '1' পাওয়া গেছে -> দ্বীপ গণনা = 1। sink() ট্রাভার্সাল শুরু।",
    ),
    DebugArrayStep(
      activeLineIndex: 4,
      ...
      explanationBn: "লাইন ৫: প্রথম দ্বীপের সংযুক্ত সব মাটি ডুবিয়ে '0' করা হলো।",
    ),
    DebugArrayStep(
      activeLineIndex: 4,
      ...
      explanationBn: "🎉 লাইন ৫: গ্রিড (2, 2) এ ২য় মাটি পাওয়া গেছে -> ২য় দ্বীপ ডুবানো শেষ! মোট দ্বীপ = 2!",
    ),
  ];

  final List<String> _gr4CodeLines = [
  ...
  final List<DebugArrayStep> _gr4Steps = const [
    DebugArrayStep(
      activeLineIndex: 1,
      array1D: [0, 1, 2, 3],
      pointer1: 0,
  ...
  List<DebugArrayStep> get _currentSteps {
    ...
    if (widget.problem.id == "gr-4") return _gr4Steps;
  ...
  List<String> get _currentCodeLines {
    ...
    if (widget.problem.id == "gr-4") return _gr4CodeLines;
  ...
  // GRAPH VISUALIZER CANVAS (gr-1, gr-2, gr-3, gr-4)
  Widget _buildGraphCanvas(DebugArrayStep step) {
    if (widget.problem.id == "gr-3" && step.matrix2D != null) {
      // 2D Grid Graph (Number of Islands)
      return Column(
        children: [
    ...
    // Graph Nodes & Edges Visualizer (gr-1, gr-2, gr-4)
