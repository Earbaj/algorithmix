...
import 'package:algorithmix/domain/models/trie_data.dart';
...
class _TrieDetailScreenState extends State<TrieDetailScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(_isEnglish ? 'Trie (Prefix Tree) Deep Dive' : 'ট্রাই (Trie / Prefix Tree) গাইড'),
        centerTitle: true,
        actions: [
          Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _buildCodeBox("Template 1: Implement Trie Class (C++ LeetCode 208)", """
struct TrieNode {
...
public:
    ...
    bool startsWith(string prefix) {
        ...
        return true;
            ...
            _buildCodeBox("Template 2: Design Add and Search Words Wildcard (C++ LeetCode 211)", """
bool searchHelp(string& word, int idx, TrieNode* curr) {
    ...
    for (int i = 0; i < 26; i++) { // Wildcard '.': try all non-null children
        if (curr->children[i] && searchHelp(word, idx + 1, curr->children[i])) return true;
    }
    return false;
}"""),
            const SizedBox(height: 16),
            _buildCodeBox("Template 3: Maximum XOR of Two Numbers Bitwise Trie (C++ LeetCode 421)", """
int findMaximumXOR(vector<int>& nums) {
  ...
  Widget _buildVisualizerTab() {
    ...
    return ResponsiveCenter(
      padding: EdgeInsets.all(hPadding),
      child: SingleChildScrollView(
        child: TrieVisualizer(isEnglish: _isEnglish),
      ),
    );
  }
  ...
  Widget _buildProblemSection(String title, List<TrieProblem> problems, Color diffColor) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
  ...
  Widget _buildProblemCard(TrieProblem p, Color diffColor) {
    return Container(
  ...
  Widget _buildMistakesTab() {
    final hPadding = Responsive.horizontalPadding(context);
    final mistakes = TrieData.getCommonMistakes(_isEnglish);
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _buildRoadmapStep(1, _isEnglish ? "Master Standard TrieNode Class Setup" : "Standard TrieNode ক্লাস তৈরি", _isEnglish ? "Define children[26] array and isEnd flag." : "`children[26]` এরে ও `isEnd` ফ্ল্যাগ ডিফাইন করুন।"),
            _buildRoadmapStep(2, _isEnglish ? "Master Basic Insert, Search & StartsWith" : "ইনসার্ট, সার্চ ও প্রিফিক্স মেথড (LeetCode 208)", _isEnglish ? "Implement insert(word), search(word), and startsWith(prefix) in O(L) time." : "O(L) সময়ে ক্যারেক্টার ট্রাভার্স করে ৩টি মেথড লজিক লিখুন।"),
            _buildRoadmapStep(3, _isEnglish ? "Master Wildcard Search with DFS Backtracking" : "ওয়াইল্ডকার্ড `.` সার্চ ও DFS (LeetCode 211)", _isEnglish ? "Recursively explore all non-null child nodes when encountering '.'." : "'.' এর জন্য ২৬টি চাইল্ড নোডে DFS লুপ চালান।"),
            _buildRoadmapStep(4, _isEnglish ? "Master Bitwise Binary Trie for Maximum XOR" : "বিটওয়াইজ Binary Trie ও ম্যাক্স XOR (LeetCode 421)", _isEnglish ? "Greedily traverse opposite bit (1 - bit) in 31-bit Trie." : "৩১-বিট ট্রিতে গ্রিডি কৌশলে বিপরীত বিট বেছে নিন।"),
            _buildRoadmapStep(5, _isEnglish ? "Master Matrix Boggle / Word Search II Pruning" : "মেট্রিক্স বোগল ও Word Search II (LeetCode 212)", _isEnglish ? "Solve Word Search II and Search Autocomplete System (LeetCode 642)." : "Word Search II ও Autocomplete System সমাধান করুন।"),
            const SizedBox(height: 24),
          ],
        ),
      ),
