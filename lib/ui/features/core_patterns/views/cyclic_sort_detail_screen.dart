...
import 'package:algorithmix/domain/models/cyclic_sort_data.dart';
import 'package:algorithmix/ui/core/theme/app_theme.dart';
import 'package:algorithmix/ui/core/utils/responsive.dart';
import '../widgets/cyclic_sort_visualizer.dart';

class CyclicSortDetailScreen extends StatefulWidget {
  const CyclicSortDetailScreen({super.key});

  @override
  State<CyclicSortDetailScreen> createState() => _CyclicSortDetailScreenState();
}

class _CyclicSortDetailScreenState extends State<CyclicSortDetailScreen>
  ...
  Widget build(BuildContext context) {
    final intro = CyclicSortData.getConceptIntro(_isEnglish);

    return Scaffold(
      backgroundColor: AppTheme.primaryDark,
      appBar: AppBar(
        title: Text(_isEnglish ? 'Cyclic Sort Deep Dive' : 'সাইক্লিক সর্ট গাইড'),
        centerTitle: true,
        actions: [
          Padding(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _buildCodeBox("Template 1: Standard 1 to N Cyclic Sort (C++ In-place O(N))", """
void cyclicSort(vector<int>& nums) {
    ...
    while (i < n) {
        ...
        if (nums[i] != nums[correctIdx]) {
            ...
            _buildCodeBox("Template 2: Find Missing Number (C++ Range 0 to N LeetCode 268)", """
int missingNumber(vector<int>& nums) {
    ...
    for (int j = 0; j < n; j++) {
        if (nums[j] != j) return j; // Missing Found!
    }
    return n;
}"""),
            const SizedBox(height: 16),
            _buildCodeBox("Template 3: Find Duplicate Number (C++ LeetCode 287)", """
int findDuplicate(vector<int>& nums) {
  ...
  Widget _buildVisualizerTab() {
    ...
    return ResponsiveCenter(
      padding: EdgeInsets.all(hPadding),
      child: SingleChildScrollView(
        child: CyclicSortVisualizer(isEnglish: _isEnglish),
      ),
    );
  }
  ...
  Widget _buildProblemsTab() {
    final hPadding = Responsive.horizontalPadding(context);
    final easy = CyclicSortData.getEasyProblems();
    final medium = CyclicSortData.getMediumProblems();
    final hard = CyclicSortData.getHardProblems();
  ...
  Widget _buildProblemSection(String title, List<CyclicSortProblem> problems, Color diffColor) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
  ...
  Widget _buildProblemCard(CyclicSortProblem p, Color diffColor) {
    return Container(
  ...
  Widget _buildMistakesTab() {
    final hPadding = Responsive.horizontalPadding(context);
    final mistakes = CyclicSortData.getCommonMistakes(_isEnglish);
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _buildRoadmapStep(1, _isEnglish ? "Master 1 to N Cyclic Swap Loop Mechanics" : "১ থেকে N সাইক্লিক সোয়াপ মেকানিক্স আয়ত্তকরণ", _isEnglish ? "Understand while(i < N) loop without unconditional i++." : "while(i < N) লুপ মেকানিক্স আয়ত্ত করুন।"),
            _buildRoadmapStep(2, _isEnglish ? "Master Single Missing Number Finding" : "একক নিখোঁজ (Missing) সংখ্যা নির্ণয়", _isEnglish ? "Scan range 0 to N to spot misplaced array indices." : "০ থেকে N সিমানায় মিসিং সংখ্যা বের করুন।"),
            _buildRoadmapStep(3, _isEnglish ? "Master All Disappeared & Duplicate Numbers" : "সকল হারানো ও ডুপ্লিকেট সংখ্যা বের করা", _isEnglish ? "Identify duplicates and missing values after cyclic sort." : "সর্ট শেষে ডুপ্লিকেট ও মিসিং শনাক্ত করুন।"),
            _buildRoadmapStep(4, _isEnglish ? "Master First Missing Positive Algorithm" : "সর্বনিম্ন পজিটিভ মিসিং সংখ্যা (LeetCode 41)", _isEnglish ? "Filter out <=0 and >N elements before cyclic swapping." : "অনিয়ন্ত্রিত সংখ্যা ফিল্টার করে সর্ট করুন।"),
            _buildRoadmapStep(5, _isEnglish ? "Master Cycle Swapping on Graphs & Couples" : "সাইক্লিক সোয়াপ ও গ্রাফ কম্পোনেন্ট", _isEnglish ? "Solve Couples Holding Hands using cyclic swap components." : "সাইক্লিক সোয়াপ দিয়ে সোয়াপ সংখ্যা বের করুন।"),
            const SizedBox(height: 24),
          ],
        ),
      ),
